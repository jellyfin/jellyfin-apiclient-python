...
This module provides a thin :class:`JellyfinClient` wrapper that handles authentication
and exposes all generated endpoints dynamically under :attr:`JellyfinClient.api`:
...
def _default_emby_authorization(
    *,
    ...
    # JellyfinClient expects this exact style for client identification.
    return (
...
class JellyfinClient:
    ...
    Usage:
        jf = JellyfinClient(base_url=..., username=..., password=...)
        jf.login()
    ...
    def __init__(
        ...
        *,
        ...
        mount_models = DEFAULTS["mount_models"] if mount_models is _DEFAULT else mount_models
        ...
        self.api = ApiNamespace(
            ...
            doc=_codeblock(
                """
                JellyfinClient API namespace (dynamically generated).
        ...
        self.models = ApiNamespace(
            "models",
            doc=_codeblock(
                """
                JellyfinClient models namespace (dynamically generated).

                This namespace exposes the generated OpenAPI models under:
                    `_generated/models/*`
                ...
                Introspection
                -------------
                dir(jf.models)              -> list available models
                help(jf.models.SomeModel)   -> show model docs / signature (if present)
                """
            ),
        )
        if mount_api:
    ...
    def login(self, *, username: str | None = None, password: str | None = None) -> JellyfinClient:
        """
    ...
    def login_with_token(self, token: str) -> JellyfinClient:
        """
    ...
    def _mount_generated_models(self) -> None:
        """
        Crawls ._generated.models and mounts model types under jf.models.<ModelName>.

        This uses the `_generated.models` package's public exports (preferred), falling
        back to scanning submodules if needed.
        """
        models_pkg_name = f"{__package__}._generated.models"
        models_pkg = importlib.import_module(models_pkg_name)

        # Prefer explicit public exports if the generator provides them.
        public_names = getattr(models_pkg, "__all__", None)
        if not public_names:
            public_names = [n for n in dir(models_pkg) if not n.startswith("_")]

        def _export(name: str, obj: Any) -> None:
            # Avoid clobbering Namespace internals
            if name.startswith("_") or name in {"__name__", "__doc__"}:
                return
            setattr(self.models, name, obj)
        ...
        # Fallback: scan modules if __init__ doesn't re-export anything useful.
        if exported_any:
            return

        if hasattr(models_pkg, "__path__"):
            for _, mod_name, is_pkg in pkgutil.iter_modules(models_pkg.__path__, models_pkg.__name__ + "."):
                if is_pkg:
                    continue
                mod = importlib.import_module(mod_name)

                # Export "public" symbols defined in that module
                for name, obj in vars(mod).items():
                    if name.startswith("_"):
                        continue

                    # Heuristic: export classes/enums/etc. that are defined in that module
                    try:
    ...
    def __repr__(self) -> str:
        return (
            f"<JellyfinClient base_url={self.base_url!r} "
...
class ApiNamespace:
    ...
    def __getattr__(self, name: str) -> Any:
        raise AttributeError(
            ...
            f"Try dir(jf.{self.__name__}) to see available entries."
        )
...
class EndpointProxy:
    ...
    def __init__(self, owner: JellyfinClient, module: Any):
        self._owner = owner
        self._module = module
    ...
    def _wrap(self, fn_name: str) -> Callable[..., Any]:
        ...
        Notes
        -----
        This function is exposed through an ``EndpointProxy`` attached to the ``JellyfinClient`` wrapper.
